...
const temp = sampahPath
export async function imageToWebp(media) {
   const tmpFileIn = `${temp}/${Date.now()}.${media?.ext || "png"}`
   const tmpFileOut = `${temp}/${Date.now()}.webp`
   ...
   try {
      ...
      fs.existsSync(tmpFileIn) ? fs.unlinkSync(tmpFileIn) : ""
      fs.existsSync(tmpFileOut) ? fs.unlinkSync(tmpFileOut) : ""
      throw e
   }
}

export async function videoToWebp(media) {
   const tmpFileIn = `${temp}/${Date.now()}.${media?.ext || "mp4"}`
   const tmpFileOut = `${temp}/${Date.now()}.webp`
   fs.writeFileSync(tmpFileIn, media.data)
   try {
      await new Promise((resolve, reject) => {
...
export async function writeExif(media, metadata) {
   ...
   if (metadata && Object?.keys(metadata).length !== 0) {
      ...
      const json = {
         "sticker-pack-id": metadata?.packId || `sansbot-${Date.now()}`,
...
export async function getTitleFromBuffer(audioBuffer, tempFilePath) {
